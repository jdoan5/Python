#!/usr/bin/env python3
"""
Predict / batch-score with a trained Fake News baseline (Stage 2).

Designed to work with the model saved by src/train.py (typically a scikit-learn Pipeline:
TF-IDF -> classifier). Supports classifiers with and without probabilities.
...
One-off text scoring:
  python src/predict.py --model artifacts/best_model.joblib --text "Breaking: ..."

Outputs (batch mode):
- y_pred: numeric label (0/1 best-effort)
- pred_label: display label ("Real"/"Fake" by default)
- y_proba_fake: probability of Fake when available (e.g., LogisticRegression, MultinomialNB)
- score_fake: raw decision score when probabilities are not available (e.g., LinearSVC)
- If --label-col is provided, also includes y_true + is_correct.

Notes:
- If you see sklearn "InconsistentVersionWarning" or runtime errors after loading a model, the
  safest fix is to retrain in the SAME virtual environment you use to run prediction/Streamlit.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Score text as Fake/Real using a trained Stage-2 model.")
    ...
    p.add_argument("--threshold", type=float, default=0.5, help="Threshold for Fake when probabilities are available.")
...
def coerce_pred_to_int(y_pred: np.ndarray, name_to_id: Dict[str, int]) -> np.ndarray:
    ...
    Unknown values become -1 (kept for debugging but still exportable).
    """
...
def predict_texts(
    ...
    # 2) Decision function (LinearSVC etc.)
    if hasattr(model, "decision_function"):
        score = np.asarray(model.decision_function(texts))

        # For binary LinearSVC this is typically (n,), and the sign depends on class ordering.
        # We'll try to align it to pos_label=1 if estimator exposes classes_.
        idx = pos_index_from_estimator(est, pos_label=1)

        if score.ndim == 2 and score.shape[1] >= 2:
            score_fake = score[:, idx] if idx is not None else score[:, 1]
        else:
            # If classes_ exists and class 1 is not the "positive" side, we'd need to flip.
            # We keep raw score but label it "score_fake" as best-effort.
            score_fake = score
...
def main() -> None:
    ...
    if args.text.strip():
        ...
        try:
            y_pred, y_proba_fake, score_fake = predict_texts(model, x, args.threshold, name_to_id)
        ...
        if y_proba_fake is not None:
            print(f"p_fake={float(y_proba_fake[0]):.4f}  threshold={args.threshold:.2f}")
