...
class TokenRevokingPasswordChangeView(PasswordChangeView):
    """Password change that also revokes the user's API token.

    Asymmetry worth knowing: Django invalidates SESSIONS on password change
    (via the session auth hash), but DRF tokens are plain DB rows that would
    survive — meaning a stolen-password attacker who already swapped it for
    a token would keep API access after the victim 'fixed' their password.
    Deleting the token closes that hole; the next /api/token/ POST mints a
    fresh one.
    """
